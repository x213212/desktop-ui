...
-- ######## Workspace rules ########
-- Give every configured role one deterministic owner workspace from the
-- first compositor frame, including role 0. Rules for disconnected outputs
-- remain dormant until that connector appears.
-- They stay non-persistent so a hotplug transaction can rename an intact
-- workspace group without colliding with an empty reservation.
for role = 0, 9 do
    local connector = desktop_ui_monitor_connector(role)
    if connector then
        hl.workspace_rule({
            workspace = tostring(
                role * desktop_ui_workspace_group_size() + 1),
            monitor = connector,
            default = true,
            persistent = false,
        })
