...
Item { // Window
    ...
    property bool capturePreferred: true
    property bool liveCaptureEnabled: true
    property bool geometryAnimationsEnabled: true
    readonly property bool captureActive: GlobalStates.overviewOpen
        && (root.capturePreferred || root.hovered || root.pressed)
    ...
    // Mask only real video previews. Icon-only overflow cards already draw
    // their own rounded rectangle, so an offscreen layer would be wasted.
    layer.enabled: root.captureActive
    ...
    Behavior on x {
        enabled: GlobalStates.overviewOpen && root.geometryAnimationsEnabled
    ...
    ScreencopyView {
        ...
        // Under high window pressure, hover requests one still frame instead
        // of a stream. This keeps identification useful without reintroducing
        // a DMA-buffer retry storm while the pointer crosses many cards.
        live: root.captureActive && root.liveCaptureEnabled
