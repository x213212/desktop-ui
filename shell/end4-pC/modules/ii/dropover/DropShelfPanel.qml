...
CachedLoader {
    id: root
    requested: GlobalStates.dropShelfOpen
    cacheDuration: 10000

    function ensurePosition() {
        if (GlobalStates.dropShelfX > 0 && GlobalStates.dropShelfY > 0)
            return
        const screen = Quickshell.screens[0]
        GlobalStates.dropShelfX = (screen?.width ?? 800) / 2
        GlobalStates.dropShelfY = (screen?.height ?? 600) / 2
    }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            if (!GlobalStates.dropShelfOpen)
                root.ensurePosition()
            GlobalStates.dropShelfOpen = !GlobalStates.dropShelfOpen
        }
        function open(): void {
            root.ensurePosition()
