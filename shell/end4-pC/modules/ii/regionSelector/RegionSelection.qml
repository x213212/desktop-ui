...
PanelWindow {
    ...
    TempScreenshotProcess {
        ...
        onExited: (exitCode, exitStatus) => {
            // Do not spawn OpenCV when grim failed or was cancelled during a
            // rapid close.  Window/layer targeting remains available and the
            // collector below still guards malformed detector output.
            if (root.enableContentRegions && exitCode === 0)
    ...
    Process {
        ...
        stdout: StdioCollector {
            ...
            onStreamFinished: {
                const output = imageDimensionCollector.text.trim();
                if (output.length === 0) {
                    // A cancelled/failed detector has no regions.  Treat that
                    // as a valid empty result instead of throwing during a
                    // rapid open/close stress cycle.
                    root.imageRegions = [];
                    return;
                }
                ...
                try {
                    ...
                    if (!Array.isArray(parsed)) {
                        console.warn("[Region Selector] Image detector returned a non-list result");
                        root.imageRegions = [];
                        return;
                    }
                    root.imageRegions = RegionFunctions.filterImageRegions(parsed, root.windowRegions);
                } catch (error) {
                    // Do not retain or print detector output: it may describe
                    // private on-screen content.  The selector still works
                    // with window/layer regions when content detection fails.
                    console.warn("[Region Selector] Ignoring invalid image-detector output:", error);
                    root.imageRegions = [];
                }
            }
        }
