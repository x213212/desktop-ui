...
Item {
    ...
    readonly property bool stopwatchPagePresented: root.presented
    ...
    function syncStopwatchFrameUpdates() {
        if (root.stopwatchPagePresented
                === root.stopwatchFrameUpdatesAcquired)
            return
        if (root.stopwatchPagePresented)
            TimerService.acquireStopwatchFrameUpdates()
        else
            TimerService.releaseStopwatchFrameUpdates()
        root.stopwatchFrameUpdatesAcquired = root.stopwatchPagePresented
    }
    ...
    onStopwatchPagePresentedChanged: root.syncStopwatchFrameUpdates()
