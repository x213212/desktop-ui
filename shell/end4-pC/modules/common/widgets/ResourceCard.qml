...
Rectangle {
    ...
    property bool highValueIsWarning: true
    property color valueColor: usageColor(value)
    ...
    ColumnLayout {
        ...
        StyledProgressBar {
            Layout.fillWidth: true
            value: root.value
            highlightColor: root.valueColor
            valueBarHeight: 6 

        }
    }

    border.width: root.highValueIsWarning && root.value > 0.9 ? 1.5 : 0
    border.color: root.highValueIsWarning && root.value > 0.9 ? Appearance.colors.colError : "transparent"
}
