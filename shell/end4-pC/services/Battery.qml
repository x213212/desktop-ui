...
Singleton {
    ...
    // UPower reports FullyCharged at 100%, so state alone loses the AC icon.
    ...
    property real sysfsEnergyFull: 0
    property real sysfsEnergyFullDesign: 0
    readonly property real sysfsHealth: sysfsEnergyFullDesign > 0
        ? Math.min(100, sysfsEnergyFull / sysfsEnergyFullDesign * 100)
        : 0
    ...
    property real health: (function() {
        ...
        return root.sysfsHealth;
    })()

    FileView {
        path: "/sys/class/power_supply/BAT0/energy_full"
        printErrors: false
        onLoaded: root.sysfsEnergyFull = Number(text().trim()) || 0
    }

    FileView {
        path: "/sys/class/power_supply/BAT0/energy_full_design"
        printErrors: false
        onLoaded: root.sysfsEnergyFullDesign = Number(text().trim()) || 0
    }
