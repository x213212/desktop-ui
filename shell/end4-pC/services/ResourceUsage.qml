...
Singleton {
    ...
    FileView {
        ...
        printErrors: false
        onLoaded: root.cpuTemp = (Number(text().trim()) || 0) / 1000
    }
    ...
    Timer {
        interval: Math.max(30000, (Config?.options.resources.updateInterval ?? 3000) * 10)
        running: true
        repeat: true
    ...
    Timer {
        ...
        repeat: true
        onTriggered: {
            // Coalesce resource and temperature reads into one wakeup.
            tempFile.reload()
            fileMeminfo.reload()
            fileStat.reload()
